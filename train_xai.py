...
class AudioClassifier(nn.Module):
    def __init__(self, num_classes=4):
        ...
        # 3-layer Shallow CNN for very small datasets
...
def train_model(model, train_loader, val_loader, criterion, optimizer, epochs=50, device='cpu'):
    ...
    for epoch in range(epochs):
        ...
        # Save best model logic (optional but good practice)
        if epoch_val_acc >= best_acc:
            best_acc = epoch_val_acc
            save_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "resnet18_xai_best.pth")
...
def main():
    ...
    # Load both microphone and stethoscope data
    ...
    # 1. Strict File-level Split (Group Split) to prevent overlap data leakage
    # We have 10 original .wav files. We assign exactly half to Train and half to Test.
    
    train_groups = [
        '20231105_220031.896329Z', # Empty 1
        '20231105_220631.588041Z', # Jamming 1
        '20231105_221100.678851Z', # Nogas 1
        '20231105_221602.995174Z', # Normal 1
        '20231105_221603.050033Z'  # Normal 2
    ]
    ...
    # Use test set as validation set as well due to extreme data scarcity
    val_files, val_labels = test_files, test_labels

    print(f"File-level Data Split - Train: {len(train_files)}, Val/Test: {len(test_files)}")

    train_dataset = PreprocessedSpectrogramDataset(train_files, train_labels, is_train=True)
    val_dataset = PreprocessedSpectrogramDataset(val_files, val_labels, is_train=False)
    ...
    # 2. Load best model for saving probabilities
    save_path = os.path.join(base_dir, "resnet18_xai_best.pth")
    ...
    # 3. Extract features and probabilities for Test Set
    all_features = []
    all_probs = []
    all_targets = []
    
    print("Extracting features and probabilities for XAI evaluation (Test Set)...")
    with torch.no_grad():
    ...
    np.save(os.path.join(base_dir, "test_features.npy"), np.vstack(all_features))
    np.save(os.path.join(base_dir, "test_probs.npy"), np.vstack(all_probs))
    np.save(os.path.join(base_dir, "test_targets.npy"), np.concatenate(all_targets))
    print("Saved test_features.npy, test_probs.npy, and test_targets.npy")
